...
public class GoodsCatalogAction extends BasicAction implements ModelDriven<GoodsCatalog> {
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public String querySecondMenuListByParentId() {
		
		List list = goodsCatalogService.querySecondMenuListByParentId(goodsCatalog.getParentCatalogId());
		
		Map<String, Object> jsonMap = new HashMap<String, Object>();
		jsonMap.put("secondMenuList", list);
		jsonObj = JSONObject.fromObject(jsonMap);
